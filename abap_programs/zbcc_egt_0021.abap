...
lo_display->set_list_header( value =  'bariscano tablo').
...
value = 'X').
"lo_cols->set_optimize(value = 'X'). neden olmuyor sor

"eğer hata olsaydı taktiği için comment out yaptım sen aç"
...
"İKİSİ ARASINDAKİ FARKA BAK

try.
lo_col = lo_cols->get_column( columnname = 'INVOICE1'  ).
lo_col->set_long_text( value =  'Yeni Fatura Düzenleyici.').
lo_col->set_medium_text( value = 'Yeni Fatura D.' ).
lo_col->set_short_text( value = 'Yeni Fat' ).
